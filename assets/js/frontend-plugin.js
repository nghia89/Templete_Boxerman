jQuery(document).ready(function ($) {
    "use strict";
    function shop_test_google_maps() {
        if ($('.shop_test-google-maps').length <= 0) {
            return;
        }
        $('.shop_test-google-maps').each(function () {
            var $this = $(this),
                $id = $this.attr('id'),
    ...
    $(document).ready(function () {
        shop_test_google_maps();
    });
    function shop_test_init_menu_toggle() {
        var contain = '.shop_test-nav-toggle';
    ...
    //
    $(document).on('click', '.button.btn-shop-the-look.bgroud-style', function () {
    ...
    // ----------shop_test_custom_scrollbar-------------------
    function shop_test_custom_scrollbar() {
        $('.shop_test-mini-cart .minicart-items').mCustomScrollbar();
        $('.shop_test-mini-cart .minicart-items').change(function () {
            $('.shop_test-mini-cart .minicart-items').mCustomScrollbar();
        });
    }

    function shop_test_custom_scrollbar_header_nav() {
    ...
    //------------------ Video Lightbox------------
    function shop_test_video() {
        $('.quick-install').simpleLightboxVideo();
    }

    // --------------------remove_class_equal--------------------------
    function shop_test_remove_class_review() {
    ...
    // -----------shop_test_details_thumd--------------------
    function shop_test_details_thumd_zoom() {
        /* ------------------------------------------------
         Arctic modal
         ------------------------------------------------ */
        if ($.arcticmodal) {
    ...
    /* TOGGLE */
    function shop_test_dropdown() {
        $(document).on('click', '.header-control .close', function () {
            $(this).closest('.shop_test-dropdown').removeClass('open');
        });
        $(document).on('click', function (event) {
            var _target = $(event.target).closest('.shop_test-dropdown');
            var _allparent = $('.shop_test-dropdown');

            if (_target.length > 0) {
                _allparent.not(_target).removeClass('open');
                if (
                    $(event.target).is('[data-shop_test="shop_test-dropdown"]') ||
                    $(event.target).closest('[data-shop_test="shop_test-dropdown"]').length > 0
                ) {
                    _target.toggleClass('open');
                    return false;
                }
            } else {
                $('.shop_test-dropdown').removeClass('open');
            }
        });
    }

    function shop_test_mobile_block() {
    ...
    // =====================slick============================
    function shop_test_init_carousel() {
    ...
    /* ---------------------------------------------
     TAB EFFECT
     --------------------------------------------- */
    function shop_test_tab_fade_effect() {
        // effect click
        $(document).on('click', '.shop_test-tabs .tab-link a', function () {
...
// ------------------owl-thumbs-----------------------------------------------
    ...
    function shop_test_countdown() {
        if ($('.shop_test-countdown').length > 0) {
            ...
            $('.shop_test-countdown').each(function () {
    ...
    $(window).resize(function () {
        ...
        shop_test_masonry()
        kt_resizeMegamenu();
        shop_test_remove_class_review();
        shop_test_details_thumd_zoom();
        shop_test_custom_scrollbar();
    });
    $(window).load(function () {
        newletter_popup();
        quickview_popup();
        shop_test_mobile_block();
        shop_test_remove_class_review();
        shop_test_custom_scrollbar()
    });
    shop_test_masonry()
    shop_test_dropdown();
    shop_test_init_carousel();
    shop_test_remove_class_review();
    shop_test_tab_fade_effect();
    shop_test_details_thumd_zoom();
    shop_test_video();
    kt_resizeMegamenu();
    shop_test_custom_scrollbar();
    shop_test_countdown();
    shop_test_init_menu_toggle();
    shop_test_custom_scrollbar_header_nav();
}); 